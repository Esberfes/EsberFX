...
public class FileWatcher implements Runnable {
    ...
    public void run() {
        LOGGER.info( "Starting FileWatcher for {0}", directoryWatched.toAbsolutePath());
        WatchKey key = null;
        while (true) {
            try {
                key = watcher.take();
                if (key != null) {
                    for (WatchEvent<?> event : key.pollEvents()) {
                        ...
                        if (watchedEvents.contains(kind)) {
                            LOGGER.info( "Invoking handle on {0}", fileName.toAbsolutePath());
                ...
                LOGGER.info( "Polling Thread was interrupted ", ex);
